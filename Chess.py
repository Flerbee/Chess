...
class Chess:
    ...
    def chess_board():
        global win
        if win:
            WinLabel = LabelFrame(root, text="Checkmate", width=200, height= 299, fg='black')
            WinLabel.place(x=400, y=200)
        else:
...
class PiecePosition:
    ...
    def capture():
        ...
        global win
        if PiecePosition.dic_chess[f"{movecell}"] == 'wking':
            print('blak win')
            win = True
        elif PiecePosition.dic_chess[f"{movecell}"] == 'bking':
            print('whit win')
            win = True
...
class Cells(DicCheck):
    ...
    def call_cell(self, r, f):
        if Cells.position is None and Cells.turn % 2 == 1:
            ...
            Cells.position = None
class CheckMate(PiecePosition):
    def checkandmate():
        if 'wking' not in PiecePosition.dic_chess:
            print("blak win")
